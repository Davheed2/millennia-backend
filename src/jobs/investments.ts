...
export async function runDailyInvestmentCron() {
	...
	for (const investment of investments) {
		...
		// Log the payout run
		try {
			await knexDb('payout_logs').insert({
				type: 'Global Daily',
				amount: totalProfitDistributed,
				recipients: recipientUserIds.size,
				status: batchStatus,
				error_message: errorMessage
			});
			console.log(`Logged payout batch: ${batchStatus}, Amount: $${totalProfitDistributed}, Recipients: ${recipientUserIds.size}`);
		} catch (logErr) {
			console.error('Failed to insert payout_log:', logErr);
		}
	}
